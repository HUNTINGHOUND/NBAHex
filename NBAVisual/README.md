...
The main reason why this app don't work wihtout the extensions is because modern browser secruity forbidden changing the origin and referrer field in HTTP requests. Therefore, I have written a simple python server that you can open and deploy on localhost:8080. The app will send request over to this server which will act like a middle man so that we can change the origin and referer header field.