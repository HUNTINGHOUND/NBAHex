            ...
            if (activeDisplay === "scatter") {
                ...
                if (toolTips) {
                    var tool_tip = d3Tip()
                        ...
                        .offset([-8, 0])
                        .html(function (d) {
                            return d.target.__data__.shot_distance + "' " + d.target.__data__.action_type;
                        });
                ...
                if (toolTips) {
                    var tool_tip = d3Tip()
                        ...
                        .html(function (d) {
                            console.log(d);
                            return d.target.__data__.makes + " / " +
                                d.target.__data__.attempts +
                                " (" + percentFormatter(d.target.__data__.shootingPercentage) + ")";
                        });
...
export class ShotChart extends React.Component {
    ...
    componentDidUpdate() {
        ...
        xmlHttp.setRequestHeader("Want", "Shot");
        xmlHttp.setRequestHeader("PlayerID", this.props.playerId);
        xmlHttp.setRequestHeader("TeamID", this.props.teamId);

        console.log("Sending request for shots");
        ...
        xmlHttp.onreadystatechange = () => {
            if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                ...
                let headers = info.resultSets[0].headers;
                let value = info.resultSets[0].rowSet;

                for (let j = 0; j < value.length; j++) {
                    let unit = {};
                    for (let k = 0; k < headers.length; k++) {
                        unit[headers[k]] = value[j][k];
                    }
                    data["shot_Chart_Detail"].push(unit);
                }

                console.log("Shot Details", data);
